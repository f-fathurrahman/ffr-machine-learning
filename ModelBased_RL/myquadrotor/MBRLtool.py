...
class Control():
    ...
    def P_dot(self,t,P,x,Wt):
        ...
        return ((self.Qb-SIGMA+P_pPhi_W+P_pPhi_W.T-self.Objective.gamma*P)*self.update_P).flatten()
...
class SysID():
    ...
    def update(self,x,x_dot,u):
        if self.ID_alg['SINDy']:
            ...
            #for i in range(self.Lib.n):
            self.Weights,self.P_rls[0] =utils.identification_RLS(self.Theta,x_dot,self.Weights,self.P_rls[0])
