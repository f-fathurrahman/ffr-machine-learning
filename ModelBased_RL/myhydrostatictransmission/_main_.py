...
output_dir_path = script_path + "/Results_{}_{}".format(version,
    list(select_ID_algorithm.keys())[list(select_ID_algorithm.values()).index(1)]
)
...
num_of_episode = 1 # duration of simulation
t_end = 5 #sampling time
h = 0.005 #0.01
# time grid points
...
# Simulation results
Sim = MBRLtool.SimResults(t, Lib, Database, SysID, Controller,
    select=select_output_graphs,
    output_dir_path=output_dir_path)
